...
@Mock(SecureRole)
class SecureRoleControllerUpdateSpec extends Specification {
    ...
    def "test not found"() {
        ...
        when:
            def control = mockSecureRoleService( true, 0 )
    ...
    private String getTemplate() {
        '<g:if test="${secureRoleInstance && edit}">OK</g:if><g:else>ERROR</g:else>'
    }

    private GrailsMock mockSecureRoleService( update = true, updateTimes = 1 ) {

        def control = mockFor( SecureRoleService )
        control.demand.get( 1 ) { Long id ->
            SecureRole.findById( id )
        }
        control.demand.update( updateTimes ) { SecureRole instance ->
            if ( update ) {
