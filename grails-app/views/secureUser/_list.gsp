        ...
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tr>
                    ...
                    <util:remoteSortableColumn property="username"
                                               title="${message(code: 'secureUser.username.label', default: 'Username')}"
                                               action="list" update="list" method="GET" params="${params}"
                                               before="\$('.panel-heading').find('.loading').show()"
                                               onComplete="\$('.loading').hide();"/>

                    <util:remoteSortableColumn property="email"
                                               title="${message(code: 'secureUser.email.label', default: 'Email')}"
                                               action="list" update="list" method="GET" params="${params}"
                                               before="\$('.panel-heading').find('.loading').show()"
                                               onComplete="\$('.loading').hide();"/>
                ...
                <g:each in="${items}" status="i" var="secureUserInstance">
                    <tr>
                        ...
                        <td>
                            <g:remoteLink action="edit" id="${secureUserInstance.id}" update="form" method="GET"
                                          before="\$('.panel-heading').find('.loading').show()"
                                          onComplete="\$('.loading').hide();"><span class="label label-success"><span
                                    class="glyphicon glyphicon-eye-open"></span></span></g:remoteLink>
                            <g:remoteLink action="delete" id="${secureUserInstance.id}" update="content"
